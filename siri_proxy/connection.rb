...
class SiriProxy::Connection < EventMachine::Connection
  ...
  if object["class"].include? "SpeechRecognized"
    puts "Siri Message received.\n".color(:yellow) if LOG_LEVEL == 1
  end
  if object["class"].include? "AssistantLoaded"
    puts "User launched Siri.\n".color(:red) if LOG_LEVEL == 1
  end
  if object["class"].include? "CommandFailed"
    puts "Unknown command.".color(:red) if LOG_LEVEL == 1
  end
  if object["class"].include? "CancelSpeech"
    puts "User cancelled Siri request.".color(:red) if LOG_LEVEL == 1
  end
  if object["class"].include? "LoadAssistant"
    #puts "User launched Siri:".color(:red) if LOG_LEVEL == 1
    #puts " " 
  end
    #puts "[Info - #{self.name}] Received Object: #{object["class"]}".color(:yellow) if LOG_LEVEL == 1
